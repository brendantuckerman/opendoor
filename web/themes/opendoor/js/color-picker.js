/**
 * @file
 * Provides UI/UX progressive enhancements on opendoor's theme settings by
...
((Drupal, settings, once) => {
  const colorSchemeOptions = settings.opendoor.colorSchemes;
  ...
  function setColorScheme({ target }) {
    ...
    if (selectedColorScheme) {
      ...
      document
        .querySelectorAll(`input[data-opendoor-custom-color]`)
        .forEach((input) => {
          input.value = input.getAttribute('data-opendoor-custom-color');
        });
    }
  }
  ...
  /**
   * Initializes opendoor theme-settings color picker.
  ...
  function initColorPicker(textInput) {
    ...
    colorInput.setAttribute(
      'data-opendoor-custom-color',
      textInput.getAttribute('data-opendoor-custom-color'),
    );
  ...
  /**
   * opendoor Color Picker behavior.
   *
   * @type {Drupal~behavior}
   * @prop {Drupal~behaviorAttach} attach
   *   Initializes color picker fields.
   */
  Drupal.behaviors.opendoorColorPicker = {
    attach: () => {
      const colorSchemeSelect = once(
        'opendoor-color-picker',
        '[data-drupal-selector="edit-color-scheme"]',
      );
      ...
      const colorTextInputs = once(
        'opendoor-color-picker',
        '[data-drupal-selector="opendoor-color-picker"] input[type="text"]',
      );
